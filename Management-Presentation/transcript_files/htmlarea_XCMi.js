...
//
// $Id: htmlarea.js,v 1.1.1.1 2003/11/21 23:14:22 hardcorelamer Exp $
...
HTMLArea.Config = function () {
	...
	this.toolbar = [
		...
		[ "copy", "cut", "paste", "space", "undo", "redo", "linebreak" ],
...
/**
,"insertimage""separator", "popupeditor", 
*/
	this.fontname = {
	...
	//    - Icon: path to an icon image file for the button (FIXME: to be changed! use one image for all buttons!)
...
HTMLArea.Config.prototype.registerButton = function(id, tooltip, image, textMode, action, context) {
	switch (typeof id) {
	    ...
	    default: alert("HTMLArea.Config::registerButton -- invalid arguments");
	}
};
...
var div = document.createElement("div");
	...
	this._statusBar = div;
	div.appendChild(document.createTextNode("Path: ")); // FIXME: I18N
...
HTMLArea.prototype.generate = function () {
	...
	if (textarea.form) {
		...
		/*
		@@@@@ old buggy, but keeping it incase errors start popping up, i know what i did @refer 15 lins below
		textarea.form.onsubmit = function() {
			editor._textArea.value = editor.getHTML();
		};
		*/
		HTMLArea._addEvent(textarea.form, 'submit', function() {textarea.value = editor.getHTML();} ); 
	...
	HTMLArea._addEvent(window, 'unload', function() {textarea.value = editor.getHTML();} );
	
	/*
	@@@@@ old buggy, but keeping it incase errors start popping up, i know what i did
...
HTMLArea.prototype.setMode = function(mode) {
	...
	switch (mode) {
		...
		if (this.config.statusBar) {
			// FIXME: I18N
			this._statusBar.innerHTML = "You are in TEXT MODE.  Use the [<>] button to switch back to WYSIWIG.";
		}
		break;
	    case "wysiwyg":
		if (HTMLArea.is_gecko) {
		...
		if (this.config.statusBar) {
			...
			this._statusBar.appendChild(document.createTextNode("Path: ")); // FIXME: I18N
...
HTMLArea.prototype._comboSelected = function(el, txt) {
	...
	switch (txt) {
		...
		break;
	    default: alert("FIXME: combo box " + txt + " not implemented");
	}
};
...
HTMLArea.prototype.execCommand = function(cmdID, UI, param) {
	...
	switch (cmdID.toLowerCase()) {
		...
		*/
			window.open("http://web.tapinternet.com/logicampus/index.php/js/main/event=fullscreen", "ha_fullscreen",
				    "toolbar=no,location=no,directories=no,status=yes,menubar=no," +
				    "scrollbars=no,resizable=yes,width=640,height=480");
...
*/
		...
		break;
		// @@@LC		
		case "insertswf": this._insertSwf(); break; 
		case "insertcontentlinks": this._insertContentLinks(); break; 
		case "insertdoclibcontentlinks": this._insertDocLibContentLinks(); break; 
		case "insertdoclibimage": this._insertDocLibImage(); break; 
...
HTMLArea.prototype._popupDialog = function(url, action, init) {
	...
	/**
	 *  @@@LC
	 *	A modified popupDialog() function to not parse the url and just trust it
	 */
	HTMLArea.prototype._popupLCDialog = function(url, action, init) {
